...
Next, we demonstrate the use of the package on the example we considered in the [Overview](Overview.Rmd). First, we simulate the data:
```{r simulate_alt}
p <- 6
n <- 9
...
```
As demonstrated in the [Overview](Overview.Rmd), the classical approach fails to reject the null hypothesis and consequently does not identify the group of variables to be correlated with the remaining variables. This primarily happens as the classical inference does not account for the fact that the hypothesis was selected from the data. Our proposed selective inference approach accounts for this:
```{r selective_alt}
