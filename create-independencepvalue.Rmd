...
**ARKA: Please tie your explanation more closely to what the function is actually doing (in terms of the definitions of `S11` as either $\P$ or $\P^C$ depending on the size of $|\P|$.  In particular, you might say something like this: Since the test statistic is identical with $\P$ and $\P^C$ interchanged, this function will swap $\P$ and $\P^C$ if needed to ensure that $|\P|\ge p/2$.**
...
When $r > 1$ & $r$ is small (i.e. in 2.), in rare cases, the numerical integration might produce a invalid (`NA` or outside $(0,1)$) approximation of the p-value (please refer to `selective_p_val_integrate()` for more details on this). We approximate the p-values using Monte Carlo approximation in these cases. In the code, we set the approximate p-value lying outside $(0,1)$ as a condition to use Monte Carlo approximation and initialise estimate of the p-value (`du` in the code) to `0`so that this condition is satisfied by default in case $r$ is large, i.e. larger that `d0` in the code. 
...
**ARKA: Please include here in latex the details of how one can go from what's shown in Section S5 and S6 to the $L$ you are constructing in the code.**
...
where, $\mathbf U \left(\mathbf x\right)$ and $\mathbf V \left(\mathbf x\right)$ are the left and right singular vectors of $\bS_{\P,\P^c}\left(\mathbf x\right)$, i.e. $\mathbf U \left(\mathbf x\right) = \left(\bS_{\P,\P}\left(\mathbf x\right)\right)^{\frac{1}{2}}\hat{\bA\left(\mathbf x\right)}$ and $\mathbf V \left(\mathbf x\right) = \left(\bS_{\P^c,\P^c}\left(\mathbf x\right)\right)^{\frac{1}{2}}\hat{\bGamma}\left(\mathbf x\right)$ respectively. We obtain $\mathbf U \left(\mathbf x\right)$ and $\mathbf V \left(\mathbf x\right)$ directly from output of `test_stat_CCA()` (`left_SV` and `right_SV` respectively). Using the dervation in S5, we have, 
\begin{equation}
\tag{***}
...
\end{equation}
Since $\mathbf S'$ only perturbs the off-diagonal component of $\bS\left( \mathbf x\right)$, we have $\mathrm{diag}\left(\mathbf S'\left(\mathbf x, \hat{\mathcal{P}}, \left\{\lambda_i\right\}_{i= 1}^{r}\right)\right) = \mathrm{diag}\left(\mathbf S\left(\mathbf x\right)\right)$. Combining this with [(**)](#eq:perturb) and [(***)](#eq:correlation) we have:
...
**ARKA: For loops are very slow in R whereas matrix multiplication is fast.  Rather than computing `temp` for each `ij` pair, I believe it should be possible to use matrix multiplication (once) and avoid the for loops entirely.  Likewise for you should be able to form the part of `L` that has $\pm1$ without for loops.**


##### Numerical integration

The quantity $p(\x,\hP)$ can be computed by numerical integration of the numerator and denominator of
...
**ARKA: To calculate `c3`, couldn't you do `sum(colSums(log(dip)))` instead?  Or for that matter even simpler: `sum(log(dip))`?  If yes, then it seems you don't need to depend on the `matrixStats` package.**
...
**ARKA: Please explain here why `du` is set to 0 at the start and end of this function.  This relates to the `try`, which should also be explained.**
...
2. Due to numerical instability, the integral in the denominator might turn out to be very close to $0$ (neglible with respect to default machine precision), especially when the true value fo the denominator is very small. In those cases to avoid returning `NA`, we force `du` to be $0$.


**ARKA: Please add some comments to the function below.**

**ARKA: Why is alpha needed in the code below?  Won't it cancel when we take the ratio?**

Since we consider the ratio of sum of the integrals, the constant part (`alpha` in the code)  cancels out from both the numerator and demnominator. We keep the `alpha` in the code to ensure that the integral in the denominator doesn't become oo small, especially when the true value of the denominator is very small. 
...
We add a test to check if `selective_p_val_numerical()` and `selective_p_val_MC()` produce identical results (within an acceptable tolerance) for small values of $r$.
```{r}
...
**ARKA: Please also add a test that verifies that `selective_p_val_beta()` and `selective_p_val_MC()` give the same answer (within an acceptable tolerance) in the r=1 case.**

**ARKA: Please also add a test that verifies that `selective_p_val_numerical()` and `selective_p_val_MC()` give the same answer (within an acceptable tolerance) in the small r case.**
