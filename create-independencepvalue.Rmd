...
3. [P-values for selected groups:](#selective) This is the heart of the package.  It computes p-values in the case that $\P$ is the result of the partitioning implemented in part 2.   Since in this case the group of variables is data-dependent, we write our tested hypothesis as 
$$
...
MC_function_classical <- function(p, rp, n){
  ...
  tilde_F_X <- tilde_W_X[,,1] %*% solve(tilde_T_X[,,1])# Winv(T)
  eigen_tilde_F_X <- eigen(tilde_F_X) # eigen decomposition of Winv(T)
  F_X_eigenvalues <- eigen_tilde_F_X$values # eigenvalues of Winv(T)
