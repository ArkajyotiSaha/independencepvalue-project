...
usethis::create_package(
  ...
  fields = list(
    ...
    Version = "0.0.1",
    Title = "independencepvalue: Computes the p-value of testing independence between groups of Gaussian variables",
    Description = "Discovers block diagonal structure from a given covariance matrix through thresholding and then tests whether a block is independent with the remaining variables.",
    `Authors@R` = c(person(
      given = "Arkajyoti",
      family = "Saha",
...
### When groups are pre-specified

We first focus on evaluating the p-value when the groups are pre-specified. For this purpose, we define two additional functions, that will come in handy later.
...
Next, we define a function to simulate from Wilks' lambda distribution:
...
### How we partition variables into groups
...
### When groups are selected with `block_diag`
...
Next, we make use of `MC_function_selective` and `dCCA` to compute the p-value for test of independece, when the group $P$ is obtained from `block_diag`.
