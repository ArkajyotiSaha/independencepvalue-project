...
Given a realization $\bx$ of $\bX$, this package implements tests of whether a particular block of the covariance matrix is zero or not, i.e.
...
where $\hat{\bLambda} := \mathrm{diag}(\hat{\lambda}_1, \hat{\lambda}_2, \ldots, \hat{\lambda}_r)$ is the diagonal matrix of the $r = r(\P):=\min\{|\P|,|\P^c|\}$ singular values of $\bS_{\P,\P^c}^W$ and $\hat{\bA}$ and $\hat{\bGamma}$ are the canonical vectors from doing a CCA on $\bX_{\P}$ and $\bX_{\P^c}$. $\hat{\lambda}_1, \hat{\lambda}_2, \ldots, \hat{\lambda}_r$ can be interpreted as the canonical correlations between the variables in $\P$ and $\P^c$. The test statistics is given by $\prod_{i = 1}^r (1 - \hat{\lambda_i}^2)$. To compute this, we define a function that given a covariance matrix $\bS$ and a group $\P$, computes and returns the test statistic along with $\bS_{\P,\P}$, $\bS_{\P^c,\P^c}$, the left and right canonical vectors of $\bS_{\P,\P^c}$, i.e. $\bS_{\P,\P}^{\frac{1}{2}}\hat{\bA}$ and $\bS_{\P^c,\P^c}^{\frac{1}{2}}\hat{\bGamma}$, since we will be using these information [when testing in the selective context](#selective). As the test statistic is identical with $\P$ and $\P^c$ interchanged, this function will swap $\P$ and $\P^c$ if needed to ensure that $|\P|\ge p/2$. This ensures that $|\P| \ge |\P|^c$. Since $r =\min\{|\P|,|\P^c|\}$, this code ensures that $r=|\P^c|$. Moreover, for the remainder of the document, we denote $p_1:=|\P|$ and $p_2 := |\P^c| = p - p_1$.
...
We also add some code to test that the function `test_stat_CCA()` works:
...
We have used the `grDevices`, `graphics`, and `igraph` for plotting the matrices and the graph, so we import these packages:
...
When $r > 1$ & $r$ is small (i.e. in 2.), the numerical integration might in rare cases produce an invalid approximation of the p-value (`du` in the code, please refer to `selective_p_val_integrate()` for more details on this) which falls outside of $(0,1)$. If `du` lies outside $(0,1)$, we approximate the p-values using Monte Carlo approximation in these cases. We initialize `du` with $0$, so that $du$ lies outside $(0,1)$ by default. 
...
where, $\mathbf U \left(\mathbf x\right)$ and $\mathbf V \left(\mathbf x\right)$ are the left and right canonical vectors of $\bS_{\P,\P^c}\left(\mathbf x\right)$ respectively, i.e. $\mathbf U \left(\mathbf x\right) = \left(\bS_{\P,\P}\left(\mathbf x\right)\right)^{\frac{1}{2}}\hat{\bA}\left(\mathbf x\right)$ and $\mathbf V \left(\mathbf x\right) = \left(\bS_{\P^c,\P^c}\left(\mathbf x\right)\right)^{\frac{1}{2}}\hat{\bGamma}\left(\mathbf x\right)$. 

We obtain $\mathbf U \left(\mathbf x\right)$ and $\mathbf V \left(\mathbf x\right)$ directly from the output of `test_stat_CCA()` (`left_SV` and `right_SV`). Using the derivation in S5, we have, 
\begin{equation}
\tag{***}
...
Since we consider the ratio of the sum of the integrals, the constant part (`alpha` in the code) cancels out from both the numerator and denominator. We keep `alpha` in the code to ensure that the integral in the denominator doesn't become too small, especially when the true value of the denominator is very small. 
...
We also add a test of the function `selective_p_val()`:
```{r}
...
We add a test to check if `selective_p_val_integrate()` and `selective_p_val_MC()` produce identical results (within an acceptable tolerance) for small values of $r$. To get higher accuracy in `selective_p_val_integrate()` prediction, we use `maxeval` $=2e5$. We use `mc_iter` $=1e5$ in `selective_p_val_MC()` to match the accuracy.
```{r}
...
We will perform simlation from dense population covariance matrix in the vignette. We add a code to generate the matrix below:
```{r}
#' Function to create a population correlation matrix for the vignette
#' 
#' This function creates a `p` x `p` matrix where the diagonal blocks are two 
#' `p/2` equicorrelation matrix with parameter `a`. The all the elements in the 
#' offdiagonal blocks of the matrix are equal to `b`. `p` has to be an even number. 
#' 
...
We will also need the package `MASS` for vignette. We add this as a suggestion in the package:

```{r}
litr:::add_text_to_file(txt = c("    MASS,"),
...
```{r}
litr::add_vignettes(c("../source-files/Overview.Rmd", "../source-files/Tutorial.Rmd"))
```
...
usethis:::use_description_field(
  "URL",
  "https://github.com/ArkajyotiSaha/independencepvalue-project/tree/main/independencepvalue"
)
```
...
Next, we build and check the package. 
